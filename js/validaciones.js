...
function validadorCien(valorIngresado) {
  //valida las extracciones que no sean divisores de 100
  var respuesta;
  valorIngresado = parseInt(valorIngresado);
  if (valorIngresado % 100 != 0) {
    enviarMensaje(
      "No se puede pedir montos de divisiones que no sean de $100."
    );
    respuesta = false;
  } else {
    respuesta = true;
...
function cancelar(valorIngresado) {
  // funciona como cancelar operacion seleccionada
